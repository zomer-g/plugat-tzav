...
export default function UpdatesManagement({ initialUpdates }: Props) {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="rounded-xl border border-dark-border bg-dark-surface p-6 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full rounded-lg border border-dark-border bg-dark-bg px-3 py-2 font-mono text-sm text-white focus:border-olive focus:outline-none"
              rows={10}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input
                ...
                className="w-full rounded-lg border border-dark-border bg-dark-bg px-3 py-2 text-white focus:border-olive focus:outline-none"
                placeholder="/images/gallery/photo.jpg או URL חיצוני"
                dir="ltr"
              />
            </div>
            <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="rounded-lg bg-dark-bg px-6 py-2 text-gray-300 hover:bg-dark-card"
            >
              ביטול
            </button>
          </div>
      ...
      {/* Updates list */}
      <p className="text-sm text-gray-500">{updates.length} עדכונים</p>

      {sortedUpdates.length === 0 ? (
        <div className="rounded-xl bg-dark-card p-8 text-center text-gray-400">
          אין עדכונים עדיין. לחצו &quot;+ עדכון חדש&quot; ליצירת הראשון.
        </div>
      ) : (
        <div className="space-y-3">
          {sortedUpdates.map((u) => (
            <div
              key={u.id}
              className="rounded-xl bg-dark-card p-4"
            >
                  ...
                  {u.tags.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {u.tags.map((tag) => (
                        <span
                          key={tag}
                          className="rounded-full bg-olive/20 px-2 py-0.5 text-xs text-olive-light"
                        >
                          {tag}
                        </span>
                      ))}
