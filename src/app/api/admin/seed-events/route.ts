...
import { getUserByEmail, getEvents, createEvent } from "@/lib/db";
...
export async function POST() {
  ...
  if (existing.length > 0) {
    return NextResponse.json({ message: `Already have ${existing.length} events, skipping seed.` });
  }
  ...
  return NextResponse.json({ message: `Seeded ${created} events successfully!` });
}
